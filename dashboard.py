...
def load_data():
    data_path = "../Data/"
    try:
        # Memuat file CSV lokal
        df_orders = pd.read_csv(f"{data_path}orders_dataset.csv.gz", compression='gzip')
        df_items = pd.read_csv(f"{data_path}order_items_dataset.csv.gz", compression='gzip')
...
# Load data dari lokal
df_orders, df_items, df_pay, df_product, df_cust = load_data()

# Jika gagal load dari lokal, tampilkan uploader
if any(df is None for df in [df_orders, df_items, df_pay, df_product, df_cust]):
    st.warning("File CSV tidak ditemukan. Silakan upload file Anda (.csv atau .csv.gz)")

    # Create file uploaders
    uploaded_files = {}
    file_names = ["orders_dataset.csv.gz", "order_items_dataset.csv.gz", 
                  "order_payments_dataset.csv", "products_dataset.csv", 
                  "customers_dataset.csv"]
    
    for file in file_names:
        if file.endswith('.csv.gz'):
            uploaded_files[file] = st.file_uploader(f"Upload {file}", type=["csv", "gz"])
        else:
    ...
    if all(uploaded_files.values()):
        try:
            df_orders = pd.read_csv(uploaded_files["orders_dataset.csv.gz"], compression='gzip')
            df_items = pd.read_csv(uploaded_files["order_items_dataset.csv.gz"], compression='gzip')
        ...
        st.stop()  # Hentikan eksekusi sampai semua file diupload

# Periksa apakah data sudah lengkap dan tidak kosong
if all(df is not None and not df.empty for df in [df_orders, df_items, df_pay, df_product, df_cust]):
    ...
    # Pastikan data preprocessing sudah selesai sebelum melanjutkan
